// Hey! Listen! Update \config\space_ruin_blacklist.txt with your new ruins!

/datum/map_template/preregistered/ruin/space
	prefix = "maps/random_ruins/space_ruins/"
	cost = 1
	allow_duplicates = FALSE

/datum/map_template/preregistered/ruin/space/zas_test
	name = "ZAS Test"
	id = "zas_test"
	description = "if this has vacuum in it, that's not good!"
	suffix = "zas_test.dmm"
	cost = 1

/datum/map_template/preregistered/ruin/space/multi_zas_test
	name = "Multi-ZAS Test"
	id = "multi_zas_test"
	description = "if this has vacuum in it, that's not good!"
	suffix = "multi_zas_test.dmm"
	cost = 1